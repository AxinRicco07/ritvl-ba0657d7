    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          size="sm"
          variant="ghost"
          className="h-8 hidden md:inline w-8 p-0 hover:bg-secondary"
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
          }}
...
const ProductCard = ({ product }: { product: PublicProduct }) => {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    addToCart({
      ...
      price: product.price,
      image:
        product.images.find((img) => img.isPrimary)?.url || "/fallback.jpg",
      sku: product.sku,
    });
  };
    ...
    <Link to={`/product/${product.id}`} className="block">
      <div className="product-card bg-white rounded-lg border border-border hover:border-primary/30 shadow-sm">
        ...
        <div className="p-4">
          ...
          <h3 className="font-medium hover:underline hover:text-primary transition-colors truncate max-w-[85%]">
            {product.name}
          </h3>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center md:gap-2">
              ...
              >
                ...
                <span className="text-xs hidden md:inline">Add</span>
              </Button>
            </div>
          </div>
        </div>
